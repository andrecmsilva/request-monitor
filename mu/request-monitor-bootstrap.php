...
function rrt_mu_snapshot() {
    ...
    if (function_exists('getrusage')) {
        ...
        $out['cpu_user_ms'] = $user;
        $out['cpu_sys_ms'] = $sys;
        $out['cpu_total_ms'] = $user + $sys;
...
function rrt_mu_write($event) {
    ...
    if ($json !== false) @file_put_contents(rrt_mu_log_file(), $json . "\n", FILE_APPEND | LOCK_EX);
}
...
function rrt_mu_safe_query() {
    ...
    foreach ($_GET as $key => $value) {
        $key = (string) $key;
        $allowed = strpos($key, 'filter_') === 0
            || strpos($key, 'query_type_') === 0
            || in_array($key, array('orderby','paged','page','product_cat','product_tag','min_price','max_price','s','rest_route'), true);
        if (!$allowed) continue;
        if (is_scalar($value)) {
            $out[$key] = substr((string) $value, 0, 500);
        } elseif (is_array($value)) {
            $vals = array();
            foreach (array_slice($value, 0, 20) as $v) $vals[] = is_scalar($v) ? substr((string) $v, 0, 500) : '[complex]';
            $out[$key] = $vals;
        }
    }
    return $out;
...
function rrt_mu_phase_durations($phases, $end) {
    $out = array();
    $previous_name = 'mu_loaded';
    $previous = $phases['mu_loaded'] ?? null;
...
function rrt_mu_shutdown() {
    if (empty($GLOBALS['rrt_bootstrap_context']) || !is_array($GLOBALS['rrt_bootstrap_context'])) return;
    $ctx = $GLOBALS['rrt_bootstrap_context'];
    $end = microtime(true);
    $resources = rrt_mu_delta($ctx['resource_start'], rrt_mu_snapshot());
    $wall_ms = ($end - $ctx['start_time']) * 1000;
    $cpu_ms = $resources['cpu_total_ms'] ?? null;
    $ratio = ($cpu_ms !== null && $wall_ms > 0) ? min(10, $cpu_ms / $wall_ms) : null;
    $last = error_get_last();
    $fatal = null;
    if ($last && in_array($last['type'], array(E_ERROR,E_PARSE,E_CORE_ERROR,E_COMPILE_ERROR,E_USER_ERROR,E_RECOVERABLE_ERROR), true)) {
        $fatal = array('type'=>$last['type'],'message'=>$last['message'],'file'=>$last['file'],'line'=>$last['line']);
    }
    $extra = array();
    if (!empty($ctx['finalizer']) && is_callable($ctx['finalizer'])) {
        try {
            $candidate = call_user_func($ctx['finalizer']);
            if (is_array($candidate)) $extra = $candidate;
        } catch (Throwable $e) {
            $extra['finalizer_error'] = $e->getMessage();
        }
    }
    $event = array_merge(array(
        'event'=>'END','version'=>'0.3.0','mu_version'=>RRT_MU_VERSION,'capture_stage'=>'mu-bootstrap',
        'timestamp'=>rrt_mu_timestamp($end),'request_id'=>$ctx['request_id'],'pid'=>$ctx['pid'],
        'method'=>$ctx['method'],'host'=>$ctx['host'],'path'=>$ctx['path'],
        'status'=>function_exists('http_response_code') ? http_response_code() : null,
        'wall_ms'=>round($wall_ms,3),'cpu_user_ms'=>$resources['cpu_user_ms']??null,
        'cpu_sys_ms'=>$resources['cpu_sys_ms']??null,'cpu_total_ms'=>$cpu_ms,
        'cpu_ratio'=>$ratio!==null?round($ratio,4):null,'classification'=>rrt_mu_classify($wall_ms,$ratio),
        ...
        'included_files'=>count(get_included_files()),'connection_aborted'=>function_exists('connection_aborted')?connection_aborted():null,
        'fatal'=>$fatal,
    ), $extra);
    rrt_mu_write($event);
}
...
$path = $request_uri !== '' ? parse_url($request_uri, PHP_URL_PATH) : null;
...
$GLOBALS['rrt_bootstrap_context'] = array(
    'start_time'=>$start,'request_id'=>$request_id,'pid'=>$pid,'method'=>$_SERVER['REQUEST_METHOD']??null,
    'host'=>$_SERVER['HTTP_HOST']??null,'path'=>$path,'deep'=>$deep,'resource_start'=>rrt_mu_snapshot(),
    'phases'=>array('mu_loaded'=>$start),'finalizer'=>null,
);
...
rrt_mu_write(array(
    'event'=>'START','version'=>'0.3.0','mu_version'=>RRT_MU_VERSION,'capture_stage'=>'mu-bootstrap',
    ...
    'method'=>$_SERVER['REQUEST_METHOD']??null,'host'=>$_SERVER['HTTP_HOST']??null,'path'=>$path,
    'query_keys'=>array_keys($_GET),'query_hash'=>$query_string!==''?substr(hash('sha256',$query_string),0,16):null,
    'safe_query'=>rrt_mu_safe_query(),'wp_action'=>$wp_action,'wc_ajax'=>$wc_ajax,
    ...
    'deep'=>$deep,'is_cron'=>defined('DOING_CRON')&&DOING_CRON,'is_ajax'=>defined('DOING_AJAX')&&DOING_AJAX,
));
