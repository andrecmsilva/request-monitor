...
// Entire idle path: one cached scalar option lookup + timestamp comparison.
...
if (!in_array($rrt_profile, array('light','hooks','deep'), true)) $rrt_profile = 'light';
...
// Light snapshots deliberately never load the global hook profiler.
...
function rrt_mu_shutdown() {
    ...
    $event=array_merge(array(
        'event'=>'END','version'=>'0.6.0','mu_version'=>RRT_MU_VERSION,'capture_stage'=>'mu-bootstrap',
...
rrt_mu_write(array(
    'event'=>'START','version'=>'0.6.0','mu_version'=>RRT_MU_VERSION,'capture_stage'=>'mu-bootstrap',
