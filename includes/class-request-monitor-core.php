...
final class Request_Monitor_Core {
    ...
    public static function sanitize_scope($scope){$out=self::default_scope();foreach(array_keys($out) as $key){if(!isset($scope[$key]))continue;$v=is_array($scope[$key])?$scope[$key]:preg_split('/[\r\n,]+/',(string)$scope[$key]);$out[$key]=array_values(array_filter(array_map('trim',$v),function($x){return $x!=='';}));}$out['types']=array_values(array_intersect(array('front','admin','ajax','rest','cron','cli'),$out['types']));$out['methods']=array_values(array_unique(array_map('strtoupper',$out['methods'])));return $out;}
    ...
    public static function install_mu(){$dir=defined('WPMU_PLUGIN_DIR')?WPMU_PLUGIN_DIR:WP_CONTENT_DIR.'/mu-plugins';if(!is_dir($dir)&&!wp_mkdir_p($dir))return new WP_Error('rrt_mu_dir','Could not create MU plugin directory: '.$dir);foreach(self::mu_files() as $file){$src=self::mu_source($file);$dst=self::mu_target($file);if(!is_file($src))return new WP_Error('rrt_mu_source','Missing bundled MU component: '.$file);if(!is_writable($dir)&&!(is_file($dst)&&is_writable($dst)))return new WP_Error('rrt_mu_permissions','MU plugin directory is not writable: '.$dir);$src_hash=@hash_file('sha256',$src);$dst_hash=is_file($dst)?@hash_file('sha256',$dst):null;if($src_hash&&$src_hash===$dst_hash)continue;$tmp=$dst.'.tmp-'.getmypid();if(!@copy($src,$tmp))return new WP_Error('rrt_mu_copy','Could not copy '.$file);@chmod($tmp,0644);if(!@rename($tmp,$dst)){@unlink($tmp);return new WP_Error('rrt_mu_rename','Could not install '.$file);}}return true;}
    ...
    public static function activate(){$r=self::install_mu();if(is_wp_error($r)){deactivate_plugins(plugin_basename(REQUEST_MONITOR_FILE));wp_die('<h1>Request Monitor activation failed</h1><p>'.esc_html($r->get_error_message()).'</p>','Request Monitor',array('back_link'=>true));}update_option(self::OPT_ENABLED,0,false);update_option(self::OPT_DEEP,0,false);add_option(self::OPT_SLOW_MS,1500,'',false);add_option(self::OPT_CALLBACK_FLOOR,5,'',false);add_option(self::OPT_MAX_MB,25,'',false);add_option(self::OPT_SCOPE,self::default_scope(),'',false);add_option(self::OPT_CAPTURE_UNTIL,0,'',true);add_option(self::OPT_CAPTURE_STARTED,0,'',false);add_option(self::OPT_CAPTURE_DURATION,0,'',false);add_option(self::OPT_CAPTURE_SESSION,'','',false);add_option(self::OPT_CAPTURE_PROFILE,'light','',false);add_option(self::OPT_VERSION,self::VERSION,'',true);update_option(self::OPT_VERSION,self::VERSION,true);self::stop_capture();}
    ...
    public static function normalize_profile($profile){$profile=strtolower(trim((string)$profile));return in_array($profile,array('light','hooks','deep'),true)?$profile:'light';}
    public static function start_capture($seconds,$profile='light'){$seconds=(int)$seconds;if($seconds<self::MIN_CAPTURE_SECONDS||$seconds>self::MAX_CAPTURE_SECONDS)return new WP_Error('rrt_capture_duration',sprintf('Capture duration must be between %d and %d seconds.',self::MIN_CAPTURE_SECONDS,self::MAX_CAPTURE_SECONDS));$r=self::install_mu();if(is_wp_error($r))return $r;update_option(self::OPT_ENABLED,0,false);update_option(self::OPT_DEEP,0,false);$profile=self::normalize_profile($profile);$started=time();$until=$started+$seconds;try{$session=gmdate('Ymd-His',$started).'-'.bin2hex(random_bytes(3));}catch(Throwable $e){$session=gmdate('Ymd-His',$started).'-'.substr(md5(uniqid('',true)),0,6);}update_option(self::OPT_CAPTURE_STARTED,$started,false);update_option(self::OPT_CAPTURE_DURATION,$seconds,false);update_option(self::OPT_CAPTURE_SESSION,$session,false);update_option(self::OPT_CAPTURE_PROFILE,$profile,false);update_option(self::OPT_CAPTURE_UNTIL,$until,true);return array('session'=>$session,'profile'=>$profile,'started'=>$started,'until'=>$until,'seconds'=>$seconds);}
    ...
    private function attach_to_mu_context(){$this->mark('regular_plugin_loaded');$GLOBALS['rrt_bootstrap_context']['finalizer']=array($this,'build_enrichment');foreach(array('plugins_loaded','after_setup_theme','init','wp_loaded','parse_request','wp','template_redirect','admin_init','rest_api_init') as $hook)add_action($hook,function()use($hook){$this->mark($hook);},PHP_INT_MAX);$profile=$GLOBALS['rrt_bootstrap_context']['profile']??'light';if($profile!=='light'){add_filter('http_request_args',array($this,'http_start'),-9999,2);add_action('http_api_debug',array($this,'http_end'),9999,5);}}
    ...
    public function build_enrichment(){$ctx=$this->has_trace_context()?$GLOBALS['rrt_bootstrap_context']:array();$profile=$ctx['profile']??'light';$deep=$profile==='deep';$escalated=!empty($ctx['auto_escalated']);$rich=$profile!=='light';return array('plugin_version'=>self::VERSION,'wordpress'=>$this->wp_context(),'included_groups'=>$rich?$this->included_groups():array(),'sql'=>($deep||$escalated)?$this->sql_summary():array('count'=>null,'total_ms'=>null,'top'=>array(),'coverage'=>'not_enabled'),'http'=>$rich?$this->http_summary():array('count'=>null,'total_ms'=>null,'top'=>array(),'coverage'=>'not_enabled'),'deep_coverage'=>array('profile'=>$profile,'sql_started_from_request_start'=>$deep,'sql_started_post_threshold'=>$profile==='hooks'&&$escalated,'http_started_from_regular_plugin_load'=>$rich,'hook_profiler_loaded'=>$rich));}
    ...
    private function http_summary(){$calls=$this->http_calls;usort($calls,function($a,$b){return $b['duration_ms']<=>$a['duration_ms'];});return array('count'=>count($calls),'total_ms'=>$this->http_total(),'top'=>array_slice($calls,0,10),'coverage'=>'from_regular_plugin_load');}
