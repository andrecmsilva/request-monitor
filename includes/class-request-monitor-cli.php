...
final class Request_Monitor_CLI_Command {
    ...
    public function capture($args,$assoc){
        ...
        if(isset($assoc['no-wait'])){WP_CLI::line('Capture is self-expiring. Check later with: wp request-monitor fingerprints --min-count=1');return;}
        sleep($seconds+1);Request_Monitor_Core::stop_capture();sleep(2);
        $active=Request_Monitor_Store::active_count();if($active)WP_CLI::warning($active.' traced request(s) are still finishing; rerun fingerprints in a few seconds for their END records.');
        $this->print_fingerprints(array(),array('mode'=>$assoc['mode']??'pattern','sort'=>$assoc['sort']??'cpu','min-count'=>$assoc['min-count']??1,'limit'=>$assoc['limit']??20));
