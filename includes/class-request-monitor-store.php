...
final class Request_Monitor_Store {
    ...
    public static function build_rows($events=null,$session=null){
        if($events===null)$events=self::read_events();$pairs=array();
    ...
    public static function fingerprint_groups($mode='pattern',$min_count=2,$slow_only=false,$session=null){
        ...
        foreach(self::build_rows(null,$session) as $row){if($row['state']!=='DONE'||empty($row[$field]))continue;if($slow_only&&!$row['slow'])continue;$key=$row[$field];if(!isset($groups[$key]))$groups[$key]=array(
            'fingerprint'=>$key,'mode'=>$mode,'count'=>0,'slow'=>0,'cpu_bound'=>0,'total_wall_ms'=>0.0,'total_cpu_ms'=>0.0,'max_wall_ms'=>0.0,'max_cpu_ms'=>0.0,'peak_memory_mb'=>0.0,
        ...
        );$g=&$groups[$key];$g['count']++;if($row['slow'])$g['slow']++;if($row['class']==='CPU_BOUND')$g['cpu_bound']++;$g['total_wall_ms']+=(float)$row['wall'];$g['total_cpu_ms']+=(float)$row['cpu'];$g['max_wall_ms']=max($g['max_wall_ms'],(float)$row['wall']);$g['max_cpu_ms']=max($g['max_cpu_ms'],(float)$row['cpu']);$g['peak_memory_mb']=max($g['peak_memory_mb'],(float)$row['memory']);unset($g);}
        foreach($groups as $key=>&$g){if($g['count']<$min_count){unset($groups[$key]);continue;}$g['avg_wall_ms']=round($g['total_wall_ms']/$g['count'],2);$g['avg_cpu_ms']=round($g['total_cpu_ms']/$g['count'],2);foreach(array('total_wall_ms','total_cpu_ms','max_wall_ms','max_cpu_ms','peak_memory_mb') as $metric)$g[$metric]=round($g[$metric],2);}unset($g);return array_values($groups);
    }
